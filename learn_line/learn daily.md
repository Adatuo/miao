...
22:53 jsonp的函数实现 小本本 面试 cors现代跨资源共享机制

## 11-22
20:03 BBS项目
20:06 .gitignore git隐藏忽略文件 不需要经常修改的代码
20:19 只有在前端交互很复杂才用了框架
20:29 多信息用post请求
20:32 为什么两个post请求 服务端和客户端
20:36 new Date().toISOString() 国际标准时间
20:37 d = new Date() 再 d.toISOString()就是当地时间
20:43 地址的跳转逻辑
20:47 为什么能拿到title 是由表单决定并解析得到的
20:49 TS是不能对动态进行检测的 不过现在也有能动态检测的产品 面试20
21:15 referer 告知服务器用户是从哪个页面跳转过来的。
21:22 评论存储
21:27 同一种数据单独存一个数组 评论 名字 ...
21:36 如何识别时区? 自己定义一个标签
21:41 模板字符串太多了怎么办? 使用模板引擎语法 pug handlebars
21:49 Handlebars lodash也有 php? 但是这两个也可以生成任何其它的代码
22:07 pug 用这个
22:26 vue里面的filter <Div>{{'owiejfweof'·|·foo·|·bar·|·baz·}}</Div> 过滤
22:28 输出到一行?
22:30 迭代
22:31 mixin
22:33 block
22:35 纯文本如果一行的开头是左尖括号（〈），那么整行都会被当作纯文本。 在pug里面写html
22:36 管道文本 文本块
22:37 包含 include 重要很多模板语法都有这个特性
22:40 模板继承
22:42 block
...
20:11 hbs
20:18 ejs
20:22 静态页面可以直接返回一个html回去,不写模板
20:26 JSON Schema Ajv(可以与express一起用) 验证数据是否符合格式 都是运行后再核对
20:30 用户信息模块
20:38 登录
20:46 表单交互逻辑图
21:05 什么是cookie 号码牌 ,可以记录任何信息
21:09 http basic authorization 但是这样会泄露用户名和密码
21:14 服务器能对cookie进行签名或者随机生成 几乎无法伪造
21:17 token令牌 为什么http不需要状态 
21:20 服务器如何给客户端发cookie 大致格式 会有多个cookie
21:35 cookie也可以在客户端和服务端删除
21:36 登出
21:40 cookieParser 解析和解密（验证）cookie的中间件加密解密cookie
21:50 签名的cookie与没签名的不在一个字段上
22:05 用cooike来find用户
22"09~12 res.locals 公用属性就可以存在这里 把东西存在这里面之后res.render就可以直接访问到 对于将请求级别的信息（如请求路径名称、已认证用户、用户设置）暴露给应用程序内渲染的模板非常有用。
22:12 评论模块
22:23 最后每一个名字字段都想清楚
22:26 删除评论
22:46 refer删除后直接调回来
22:49 前端实现删除
22:53 删除事件绑定在body比较好

## 11-24
20:02 bbs数据存到数据库
20:03 数据库的类型 关系型 日志型 缓存型 文档型
20:07 数据库是对海量数(小片段数据)据进行结构化存储的软件系统
20:09 sql Structured Query·Language 结构化查询语言
20:12 数据库软件的一般进程图
20:15 数据库的一般模型(都是通过网络模型沟通)
20:16 SQLite模型(非网络模型)
20:20 特别适合于被嵌入任何一个软件里，成为它依赖的一个第三方库
20:24 SQLite小巧大 单文件4T
20:24 SQLite安装
20:28 SQLite win安装
20:30 linux环境下的使用
20:31 '透明的'概念
20:33 sqlite3 [].db 创建数据库文件
20:36 基本表创建
20:39 设置输出模式
20:40 结尾打分号 ;
20:42 winpty ./sqlite3.exe [].sqlite3 windows下需要添加
20:44 box模式
21:02 w3school sql文档
21:06 distinct 去重
21:08 表头必须区分大小写
21:09 组合去重
21:13 count计算某一列多少行
...
21:16 <> 不等于
21:20 LIKE IN
21:20 ORDER BY DESC(降序,默认升序)
21:23 OR
21:26 表与列的填写
21:27 null
21:29 update delete select top
21:36 limit 2,2
21:36 聚集函数 数学运算都要指出列名 Min Max 
21:40 Like%(多个) _(一个)
21:42 为什么不用正则表达式? 太费时间
21"43 Wildcards
21:44 ??
21:45 AS改名 还可以把其它列中的值拼出来as成一个新的列
21:47 UNION(默认自动去重) ALL 
21:49 GROUP BY 分组
22:06 HAVing 对 GROUP BY过滤
22:09 INTO 插入
22:11 Case
22:11 Null Fuction
22:12 Stored Procedures 存储过程用SQL写函数
22:13 -- /* */注释
22:13 Database 关系型数据库表示多个文件夹 而sqlite里这个概念被弱化了，5为一个sqlite的数据库文件就是一个"database"，里面包含
多个表 小本本
22:17 Drop DB BackupDB 
22:18 Creat Table
22:20 SQLite Data Types 使用的是动态类型数据 ??
22:24 现在也能在严格模式下创建静态类型
22:35 Drop Alter(也可以修改类型) table
22:37 Constraints限制
22:40 键 外键(图) 主键
22:43 NOT Null
22:44 Unique
...
20:12 SQL Join
20:13 交叉连接 笛卡尔连接
20:18 一般配合on一起使用 20:20一般只用=号
20:23 inner join(默认)
20:24 各种jointutu
20:25 left join right join full join self join没有的话会留空
20:39 view视图 
21:41 .schema 是一个用于获取数据库结构信息的方法。它返回的是数据库中所有表、索引、触发器等对象的 SQL 定义
21:42 注入 Injection
21:47 Index索引 
21:47~54 为什么要用索引 方便同一查找
20:57 BST(排序二叉树)来存储索引
21:00 索引需要创建 还可以创建多列联合索引
21:14 better-sqlite3
21:29 并发与并行 小本本
21:20 RDBMS 关系型数据库管理系统
21:32 node里面的sqlite 命令文档(但是还处于开发状态,暂且不用)
21:33 prepare 这样不会产生注入,一模一样的的是不会再次编译的
21:41 启用better-sqlite3
22:00 在代码中使用sqlite存储数据
22:10 表格式和属性有些错误,需要重置
22:28 run会自动返回一个rowID 
22:38 错误对象通常包含以下几个常见属性：
      name: 错误的名称（例如，TypeError, ReferenceError, SqliteError 等）。
      message: 错误的详细描述信息。
      stack: 错误发生时的堆栈跟踪信息，通常用于调试。
22:43 主键 ID 不会重用

## 11-27
20:35 验证码功能
20:41 canvas与svg的选择
20:45 node-canvas 其实就是在操作buffer内存里面的位置
20:47~53 captcha解决方式
21:00 js binding python binding 指其它语言实现的某种功能在js/py语言中以几乎相同的形式出现，但函数实际调用的是更底
层的语言版本的实现，上层语言自身什么也没做 小本本
21:02 svg-captcha
21:05 图片滑动验证码 human验证码(原理是记录鼠标移动轨迹,时间等等)
21:14 交通图片识别之前是书本识别
21:34 验证码的对比,放在一个变量里面
21:36 生成出来的验证码可以先放在一个变量里面,但是需要指向最后一个生成的二维码
21:42 用cookie来识别是谁请求的二维码
21:45 下发一个sessionId cookie来区分
21:58 cookie session会话 token令牌 面试
22:01 前端cookic操作小本本
22:09 以游戏为例子,密码输出多次,验证码才显示
22:18 验证码点击更新
22:32 验证码也有现成的中间件
22:37 查询当前用户的评论列表功能未写
22:38 忘记密码功能
22:48 分享链接的一些细节
22:51 发送电子邮件的库 Nodemailer
22:40 修改密码功能
23:16 不要明文密码防止脱库 md5(pwd)存储 
23:19 用MD5存的时候可能会被彩虹表攻击 因为一些密码的MD5是一样的
23:20 用md5((md5[password]) + md5(salt) +salt) 加盐 salt可以是随机的 加盐后哈希存储

## 11-28
20:02 密码加盐
20:05 加密与哈希不太一样
20:07 md5函数包 自带的是createHash流实现的不太好用
20:14 刷库 业务逻辑以及数据库表结构发生变化后，有些时候需要将数据库的数据也批量更新为匹配新的业务逻辑的数据，这个时候就需要刷库了 这个操作非常敏感，因为它会对数据进行永久性变更，一旦操作错了，损失是无法估量的！所以操作前一定要备份数据 小本本
20:18 刷库代码 最好测试一遍
20:30 删除帖子
20:44 Seqquelize 可以实现更高级的删除
20:59 翻页功能 limit offset
21:16 注册,头像功能
21:19 表单默认只传输文件名字 需要enctype="multipart/form-data"> 
21:21 传统表单的头解析
21:24 需要再写一个中间件解析 enctype 官方推荐multer
21:33 业务下,需要验证上传的是不是图片
21:37 图片扩展名的 multer存储
21:41 传上去后需要用对应中间件读出来图片
22:02 图片上传补充 upload Ant为例表单
22:08 修改个人信息，跟注册差不多，只不过id不能改 用户详情 板块分类 指点
22:17 如果没有使用模板引擎的话会inner=''会变成空页面
22:20 网站常见的安全漏洞 小本本
22:23 跨站脚本攻击 xss 此处跨站指跨越整个站点，即可能影响整个站点 转移后显示而不是直接显示
22:25~35 跨站请求伪造 此处跨站指跨越两个不同和站点 csrf 因为以前浏览器会带有cookie转发才能成功,现在已死 以前会带一个其它站点无法获取的cookie
22:38 注入 SQL Inject sql 使用sql语句占位符解决

## 11-29
20:03 KOA框架
20:05 只有app.use(async(ctx,next)=>{})
20:07 请求与响应都在ctx上 
20:09 可以await next koa与express的重要不同 小本本
20:10 KOA next详解 
20:11~17 next后面有代码何时执行,在await next()之后执行
20:18 await next()运行图 洋葱模型图
20:26 为什么express不能像KOA一样头尾计算时间?(因为中间件可能没有调用next)
20:28 ctx上的属性
20:30 ctx.state 其它中间件都能读到
ctx.cookie ctx.throw ctx.respond
20:35 Request/Respond aliases请求别名
20:38 router中间件获取参数
20:39 其它功能均通过第三方中间件实现
20:43 如何写中间件
20:48 egg 第三方中间件实现简介
21:01 数据库索引 这是按照插入顺序排列的
21:03 orm 数据库设计 索引
21:03 索引 四角号码 会变成树形查找 
21:06~ 数据库设计
21:16 数据库范式 一级范式 
21:19 一对多 一对一 多对多 文件本本
21:25 多对多的保存方式 是没法在两张表查询的 要单独建立多对多关系
21:28 数据库设计不应该在需求不改变的情况下增加表(上线后)
21:29 orm 将表与表中行的关系与类与类的实例的关系,进行对应，即为对象关系模型 sequelize就是这样的框架 有时间就再看一遍
21:36 memory sqlite没有传数据库文件会在内存里面开一个数据库
21:38 sequelize model
22:03 transaction事务 
22:07 事务操作 原子操作
22:10 transaction 事务中的数据库操作要么全部完成,要么全部不完成(相当于没发生)
22"11 数据库锁 行级锁,表级锁 防止修改数据
22:13 数据库系统设计(system design) 面试 后端或者全栈才会问
22:14 系统设计 面试
22:20 网盘演示(原生js)
