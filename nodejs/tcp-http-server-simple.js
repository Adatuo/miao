...
server.on('connection',(conn) =>{
  ...
  conn.on('data',data => {
    ...
    let [method,url] = firstLine.split(' ')//拆分请求行中的请求方法和路径,这里的url只有path
   
    let IP = `http://localhost:8005${url}`  //完整地址,用反斜杠是因为${}模板字符串
    let urlObj = new URL(IP);

    console.log(url)
    //console.log("请求体"+body);//name=asd&message=yes%3F POST有请求体GET没有

    if (method == "POST" && urlObj.pathname == "/example/message.html") {//post也必须在相同网址
      let message = parseQueryString(body)
      messages.push(message)//添加进数组
      // return 不能,结束了就无法发送消息
    }
    
    if (urlObj.pathname == "/example/message.html") {//绝对路径与相对路径
      //响应
